...
if ($conn->connect_error) {
 ...
 if(isset($_POST['btnsave']))
 {
  ...
  if(!isset($errMSG))
  {
   $sql = "INSERT INTO pizza(Image,Iname,Price) VALUES('$imgFile','$iname','$Price')";
   $conn->query($sql);
   if($conn)
   {
    echo "new record succesfully inserted ...";
    header("refresh:2;pizza.php"); // redirects image view page after 5 seconds.
   }
   else
   {
...
$iname = $_POST['Iname'];
$sql = "delete from pizza where Iname = '".$iname."'";
$conn->query($sql);
if($conn){
echo "Data Deleted succesfully...";
header("refresh:2;pizza.php");}
else{
echo "Fail to delete";}
}
?>
...
$sql =("UPDATE pizza SET Image = '$imgFile', Iname = '$iname', Price = '$Price' WHERE  Iname = '".$iname."'");
$conn->query($sql);
if($conn){
