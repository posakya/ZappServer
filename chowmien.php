...
if ($conn->connect_error) {
 ...
 if(isset($_POST['btnsave']))
 {
  ...
  if(!isset($errMSG))
  {
   $sql = "INSERT INTO chowmien(Image,Iname,Price) VALUES('$imgFile','$iname','$Price')";
   $conn->query($sql);
   if($conn)
   {
    echo "new record succesfully inserted ...";
    header("refresh:2;chowmien.php"); // redirects image view page after 5 seconds.
   }
   else
   {
...
$iname = $_POST['Iname'];
$sql = "delete from chowmien where Iname = '".$iname."'";
$conn->query($sql);
if($conn){
echo "Data Deleted succesfully...";
header("refresh:2;chowmien.php");}
else{
echo "Fail to delete";}
}
?>
...
$sql =("UPDATE chowmien SET Image = '$imgFile', Iname = '$iname', Price = '$Price' WHERE  Iname = '".$iname."'");
$conn->query($sql);
if($conn){
