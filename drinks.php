...
if ($conn->connect_error) {
 ...
 if(isset($_POST['btnsave']))
 {
  ...
  if(!isset($errMSG))
  {
   $sql = "INSERT INTO drinks(Image,Iname,Price) VALUES('$imgFile','$iname','$Price')";
   $conn->query($sql);
   if($conn)
   {
    echo "new record succesfully inserted ...";
    header("refresh:2;drinks.php"); // redirects image view page after 5 seconds.
   }
...
$iname = $_POST['Iname'];
$sql = "delete from drinks where Iname = '".$iname."'";
$conn->query($sql);
if($conn){
...
$sql =("UPDATE drinks SET Image = '$imgFile', Iname = '$iname', Price = '$Price' WHERE  Iname = '".$iname."'");
$conn->query($sql);
if($conn){
